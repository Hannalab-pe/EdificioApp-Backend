...
} from 'apps/people-service/src/Dtos';
...
export class InmobiliariaService implements IInmobiliariaService {
  ...
  async create(
    ...
    try {
      ...
      const savedInmobiliaria =
        await this.inmobiliariaRepository.save(newInmobiliaria);
      ...
      if (error.code === '23505') {
        // Violation de constraint único en PostgreSQL
  ...
  async update(
    id: string,
    data: UpdateInmobiliariaDto,
  ): Promise<BaseResponseDto<Inmobiliaria | null>> {
    try {
      // 1. Verificar que la inmobiliaria existe y está activa
      const inmobiliaria = await this.inmobiliariaRepository.findOne({
        where: { id, activa: true }, // Solo inmobiliarias activas
      ...
      if (data.ruc && data.ruc !== inmobiliaria.ruc) {
        ...
        if (existingByRuc && existingByRuc.id !== id) {
          return BaseResponseDto.error(
            'Ya existe una inmobiliaria con ese RUC',
            HttpStatus.BAD_REQUEST,
          );
        }
      }
      ...
      if (error.code === '23505') {
        // Violation de constraint único en PostgreSQL
        return BaseResponseDto.error(
          'Violación de restricción única (RUC o razón social duplicados)',
          HttpStatus.BAD_REQUEST,
        );
      }
  ...
  async reactivate(id: string): Promise<BaseResponseDto<Inmobiliaria>> {
    try {
      ...
      if (!inmobiliaria) {
        return BaseResponseDto.error(
          'Inmobiliaria no encontrada o ya está activa',
          HttpStatus.NOT_FOUND,
        );
      }
      ...
      const fullReactivatedInmobiliaria =
        await this.inmobiliariaRepository.findOne({
          where: { id },
          relations: ['propietarios'], // Incluir propietarios para respuesta completa
        });
