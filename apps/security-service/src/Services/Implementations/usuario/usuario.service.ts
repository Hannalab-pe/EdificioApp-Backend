...
import { IUsuarioService, CreateUsuarioDto, UpdateUsuarioDto, UsuarioQuery } from '../../Interfaces/usuario/iusuario.service';
...
export class UsuarioService implements IUsuarioService {
    ...
    async create(data: CreateUsuarioDto): Promise<Usuario> {
        ...
        if (userWithDocument) {
            throw new ConflictException('El documento de identidad ya está asociado a otro usuario');
        }
    ...
    async findAll(query: UsuarioQuery = {}): Promise<{ usuarios: Usuario[], total: number, page: number, totalPages: number }> {
        ...
        // Filtros
        if (email) {
            queryBuilder.andWhere('usuario.email ILIKE :email', { email: `%${email}%` });
        }
        ...
        if (nombre) {
            queryBuilder.andWhere('(usuario.nombre ILIKE :nombre OR usuario.apellidos ILIKE :nombre)',
                { nombre: `%${nombre}%` });
        }
    ...
    async update(id: string, data: UpdateUsuarioDto): Promise<Usuario> {
        ...
        if (data.documentoIdentidadId && data.documentoIdentidadId !== usuario.documentoIdentidadId) {
            const userWithDocument = await this.usuarioRepository.findOne({
                where: { documentoIdentidadId: data.documentoIdentidadId }
            });
