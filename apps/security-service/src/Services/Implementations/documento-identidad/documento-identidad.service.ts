...
export class DocumentoIdentidadService implements IDocumentoIdentidadService {
    ...
    async create(data: CreateDocumentoIdentidadDto): Promise<DocumentoIdentidad> {
        ...
        if (existingDoc) {
            throw new ConflictException('Ya existe un documento con el mismo tipo y número');
        }
    ...
    async findAll(query: DocumentoIdentidadQuery = {}): Promise<{ documentos: DocumentoIdentidad[], total: number, page: number, totalPages: number }> {
        ...
        if (numero) {
            queryBuilder.andWhere('documento.numero ILIKE :numero', { numero: `%${numero}%` });
        }
        ...
        if (paisEmision) {
            queryBuilder.andWhere('documento.paisEmision = :paisEmision', { paisEmision });
        }
    ...
    async findByTipoAndNumero(tipo: TipoDocumento, numero: string): Promise<DocumentoIdentidad> {
        const documento = await this.documentoRepository.findOne({
            where: { tipo, numero }
        });
    ...
    async update(id: string, data: UpdateDocumentoIdentidadDto): Promise<DocumentoIdentidad> {
        const documento = await this.findOne(id);

        if ((data.tipo && data.tipo !== documento.tipo) || (data.numero && data.numero !== documento.numero)) {
            const existingDoc = await this.documentoRepository.findOne({
                where: {
